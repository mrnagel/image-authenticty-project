...
# Main taken from inference.py for DDA
def inference(input_image):
    ...
    device = 'cuda:0' if torch.cuda.is_available() else 'cpu' # updated for flexibility
    ...
    image_path = input_image # Changed from test.jpg
...
def main():
    ...
    parser.add_argument("--input_path", '-i', type=str, help="The path of the input image")
    args = vars(parser.parse_args())

    output = inference(args['input_path'])
    p_fake = output[0].item()
    p_real = 1 - p_fake

    os.makedirs(os.path.dirname('/model_outputs/dda/results.npz'), exist_ok=True)
    np.savez('/model_outputs/dda/results.npz', p_fake=p_fake, p_real=p_real)
